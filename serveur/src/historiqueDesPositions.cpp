...
bool8 HistoriqueDesPositions::calculer(sint32 heure, float16* x, float16* y, float16* angle)
{
    ...
    else // Deux positions connues ou plus
    {
        ...
        else if (heure < this->positions[dernierePosition].heure)
        {
            ...
            while(TRUE) // Parcours l'historique en partant de la fin
            {
                if (this->positions[i].heure <= heure) // Detection de la position immediatement avant
                {
                    ...
                    *x = this->positions[i].x + (this->positions[i+1].x - this->positions[i].x)*(heure-this->positions[i].heure)/(this->positions[i+1].heure-this->positions[i].heure);
                    *y = this->positions[i].y + (this->positions[i+1].y - this->positions[i].y)*(heure-this->positions[i].heure)/(this->positions[i+1].heure-this->positions[i].heure);
                    ...
                    {
                        *angle = this->positions[i].angle + (this->positions[i+1].angle - this->positions[i].angle)*(heure-this->positions[i].heure)/(this->positions[i+1].heure-this->positions[i].heure);
                    }
                    else if (this->positions[i+1].angle - this->positions[i].angle >= 180) // Au voisinage de +/-180 degres, vers la gauche
                    {
                        *angle = this->positions[i].angle + ((this->positions[i+1].angle-360) - this->positions[i].angle)*(heure-this->positions[i].heure)/(this->positions[i+1].heure-this->positions[i].heure);
                    }
        ...
        else
        {
            ...
            *x = this->positions[dernierePosition].x + ( (this->positions[dernierePosition].x - this->positions[dernierePosition-1].x) / (this->positions[dernierePosition].heure - this->positions[dernierePosition-1].heure) ) * (heure - this->positions[dernierePosition].heure);
            *y = this->positions[dernierePosition].y + ( (this->positions[dernierePosition].y - this->positions[dernierePosition-1].y) / (this->positions[dernierePosition].heure - this->positions[dernierePosition-1].heure) ) * (heure - this->positions[dernierePosition].heure);

            // Extrapolation de l'angle
            *angle = this->positions[dernierePosition].angle + ( (this->positions[dernierePosition].angle - this->positions[dernierePosition-1].angle) / (this->positions[dernierePosition].heure - this->positions[dernierePosition-1].heure) ) * (heure - this->positions[dernierePosition].heure);
            if (*angle >= 180)
            {
                *angle -= 360;
            }
